package tr.com.ykartal.stemmer;
...
public class Stemmer {
  ...
  /**
   */
  public static void runStemmer(String sourceFolder, String destinationFolder) {
    char[] w = new char[501];
    ...
    for (int i = 0; i < fileArr.length; i++)
      try {
        in = new BufferedReader(new java.io.FileReader(fileArr[i].getPath()));
        File outDir = new File(destinationFolder);
        outDir.mkdir();
        File outFile = new File(destinationFolder + "/" + fileArr[i].getName());
        if (!outFile.exists()) {
        ...
        try {
          ...
          System.out.println("error reading " + fileArr[i]);
          break;
        }
        out.close();
      } catch (FileNotFoundException e) {
        System.out.println("file " + fileArr[i] + " not found");
        break;
      } catch (IOException e) {
