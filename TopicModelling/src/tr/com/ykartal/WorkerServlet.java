...
/**
 * An example Amazon Elastic Beanstalk Worker Tier application. This example requires a Java 7 (or
 * higher) compiler.
 */
public class WorkerServlet extends HttpServlet {
   ...
   * {@code AwsCredentials.properties} file if found on the classpath, otherwise will attempt to
   * obtain credentials based on the IAM Instance Profile associated with the EC2 instance on which
   * it is run.
   */
  private final AmazonS3Client s3 = new AmazonS3Client(
      new AWSCredentialsProviderChain(new InstanceProfileCredentialsProvider(), new ClasspathPropertiesFileCredentialsProvider()));

  /**
   * This method is invoked to handle POST requests from the local SQS daemon when a work item is
   * pulled off of the queue. The body of the request contains the message pulled off the queue.
   */
  @Override
